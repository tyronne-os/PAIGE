...
def _event(
    ...
    """
    record: dict[str, object] = {"ts": ts, "event": name, "issue": issue}
    if repo is not None:
        record["repo"] = repo
...
def test_intact_record_survives_being_glued_to_a_torn_one(workspace: Path) -> None:
    ...
    result = fold.fold_pipeline(repo="acme/widget")
    ...
    tricky = json.dumps(
        {
            ...
            "issue": 7,
            # Stamped like every event the writers emit, so a scoped fold admits it
            # and the assertion below is about the GLUE rather than about scoping.
            "repo": "acme/widget",
...
def test_a_re_entered_item_is_in_flight_again(
    ...
    implement = next(
        s for s in fold.fold_pipeline(repo="acme/widget").steps if s.key == "implement"
    )
