/**
 * Screenshot + measurement runner for capture/workflow-completion-overflow.html.
 *
 ...
 * the expanded result inside a bounded, inner-scrolling body; `before` must
...
const OUT = process.argv[3] || '../temp-screenshots/workflow-completion-overflow'
...
const BODY = '[data-testid="workflow-completion-body"]'
...
for (const theme of ['dark', 'light']) {
  for (const scene of ['before', 'after']) {
    ...
    try {
      ...
      await page.goto(`${BASE}/capture/workflow-completion-overflow.html?scene=${scene}&theme=${theme}`, {
        waitUntil: 'load',
      })
      await page.waitForSelector('[data-capture-root]', { timeout: 15000 })
      // The card mounts FOLDED (result behind "Show result"), so expand the
      // disclosure the way a reader would.
      const toggle = page.locator('[data-capture-root] button[aria-expanded="false"]')
      ...
      if (scene === 'after') {
        ...
        if (!m.innerScrolls) {
          frameFailed++
          console.error(`FAIL ${name}: result fits ${m.bodyH}px — too short to prove the inner scroll`)
        }
