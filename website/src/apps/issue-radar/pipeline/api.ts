...
// A repository's identity comes from the HOST APP, not from a second declaration
// here. This module carried its own `SourceProvider` and `RepoRef` while the
// pipeline was a standalone app, so it could compile without depending on Issue
// Radar -- a reason this change removes, since the pipeline is now one of Issue
// Radar's dashboards and its views already import from that app. Two byte-identical
// declarations of the type that crosses the seam is the drift the shared
 ...
 * re-exported rather than restated. It mirrors `crew_store.PHASES` on the backend,
 * and the pure fold derives the on-spine subset from it, so a second copy here
 * would be a second thing to keep in step with the enum. Verified identical to the
 * copy this replaces, item for item, before collapsing them. */
export { CREW_PHASES } from '../api'
export type { CrewPhase } from '../api'
import { CREW_PHASES } from '../api'
import type { CrewPhase } from '../api'
...
// There is deliberately NO repository preference here any more, and no coercion
// helper guarding one. The pipeline is a dashboard inside Issue Radar and is
// HANDED the active repository by its host, so every mechanism for
// reconstruct that choice from outside is gone: this app's own localStorage key,
// the read of Issue Radar's key as a first-visit seed, the `RepoRef` coercion
// that existed only because those reads were untrusted, and the connected-repo
// fetch that validated a remembered choice against the backend. Two surfaces
// guessing, through storage, at a value one pane away was the defect; the answer
// is passed in now, so the guessing is deleted rather than hardened.
...
/** Put a repository's WHOLE identity on a pipeline request.
 *
 * owner/repo alone is not a repository: the same slug on public GitHub and on a
 * self-hosted forge are different repositories. The pipeline's data is keyed on
 * owner/repo only, so the backend refuses anything that is not public GitHub --
 * which it can only do if the request actually SAYS which forge it means. Sending
 * a bare pair would silently claim "public GitHub" on the caller's behalf.
 *
 * The repository is REQUIRED, matching the routes: every repo-scoped handler answers
 * a bare request with 400 `repo_required`, so an optional parameter here would only
 * let a caller build a request that cannot succeed.
 *
 * Used by every repo-scoped call in this module -- `overview`, `step` and
 * `itemSessions` -- so a fourth cannot quietly re-drop the identity by spelling the
 * pair out inline. That is not hypothetical: spelling it inline is how provider and
 * host went missing from two of these three in the first place.
 */
function setRepoIdentity(q: URLSearchParams, repo: RepoRef): void {
  q.set('owner', repo.owner)
  q.set('repo', repo.repo)
  if (repo.provider) q.set('provider', repo.provider)
  if (repo.host) q.set('host', repo.host)
}
...
// ---------------------------------------------------------------------------
// L0 / L1 / L2 — the pipeline dashboard's OWN three read endpoints.
//
// These hit the pipeline routes nested under Issue Radar
// (`/api/apps/issue-radar/pipeline/*`, folded by `pipeline_fold.py`), not the
// crew seam above. The `to_dict()` payloads in that module ARE the contract
// mirrored here.
//
...
/** The pipeline's own base, DERIVED from the host app's base rather than spelled
 * again. Because the clients below are forward-tolerant, a base that drifted
 * from what the backend mounts would not surface as an error — it would render
 * as a pipeline that has never run. One spelling makes that impossible. */
...
async function getObjectOrThrow(path: string): Promise<Record<string, unknown>> {
  ...
  if (!r.ok) {
    ...
    // that sees only a status has to re-derive the rule to explain the refusal, and
    // a second copy of a rule is a copy that drifts. Reading the code instead makes
    // the refusal itself the source. Best-effort: a non-JSON error body still yields
    // a useful throw.
    let code = ''
    try {
      const body: unknown = await r.json()
      const o = asObject(body)
      if (o && typeof o.code === 'string') code = o.code
    } catch {
      /* a body that will not parse simply has no code */
    }
    const err = new Error(`request failed with status ${r.status}`) as ApiError
    err.status = r.status
...
/** An error from a pipeline request, carrying the backend's own code when it sent one. */
 ...
 */
export function isUnsupportedForge(err: unknown): boolean {
...
export const autoTriagePipelineFoldApi = {
   ...
   */
  overview: async (hours: number | undefined, repo: RepoRef): Promise<OverviewResponse> => {
    ...
    // The repository is required and travels whole. A bare or partial request is a
    // 400 rather than a widening, so there is no "every repository" reading left to
    // describe -- the census this endpoint returns is whole-trail on a SCOPED call
    // anyway, which is why nothing was lost by requiring it.
    setRepoIdentity(q, repo)
  ...
  step: async (query: StepQuery): Promise<StepResponse> => {
    const q = new URLSearchParams({ step: query.step })
    setRepoIdentity(q, query)
   ...
   * failure.
   */
  itemSessions: async (number: number, repo: RepoRef): Promise<ItemSessionsResponse> => {
    const q = new URLSearchParams({ number: String(Math.trunc(number)) })
    // The repository matters most here of anywhere: issue numbers are
    // per-repository, so an item that merely shares a number could otherwise
    // return another repository's sessions -- and another repository's CREDIT
    // COSTS. There is no default shard to fall back to: the backend requires the
    // repository and refuses a bare request, which is why it is required here too.
    setRepoIdentity(q, repo)
