...
import type { RepoRef, StepItem } from '../api'
...
// Deliberately NOT kirodotdev/KiroCrew. The PR link used to be hardcoded to that
// repository, so a fixture naming it would let the old and the correct behaviour
// produce byte-identical hrefs and the assertions below would pass either way.
const TEST_REPO: RepoRef = { owner: 'acme', repo: 'widgets', provider: 'github' }
...
describe('StepItemsTable — PR column', () => {
  ...
  it('builds the href from the ROW\u2019S repository, not a fixed one', () => {
    // A PR number is per-repository, so a fixed target does not fail loudly -- it
    // opens a DIFFERENT, real pull request and looks entirely plausible. This is the
    // same misattribution the shard-per-repository queue exists to prevent, one
    // layer out in the UI.
    renderTable([item({ number: 4820, pr: 5127 })])
    const href = within(rowFor(4820)).getByRole('link').getAttribute('href') ?? ''
    expect(href).toContain('acme/widgets')
    expect(href).not.toContain('kirodotdev/KiroCrew')
  })

  it('uses the forge\u2019s own grammar rather than assuming GitHub\u2019s', () => {
    // Delegated to Issue Radar's link builder instead of composed here: GitLab
    // nests a project's pages under `/-/` and calls it `merge_requests`, so a
    // hand-built `/pull/<n>` would 404 on the very repositories this board can be
    // pointed at once the backend admits them.
    renderTable([item({ number: 4820, pr: 5127 })], {
      repo: { owner: 'group/sub', repo: 'thing', provider: 'gitlab' },
    })
    const href = within(rowFor(4820)).getByRole('link').getAttribute('href') ?? ''
    expect(href).toContain('/-/merge_requests/5127')
    expect(href).not.toContain('/pull/5127')
  })
