...
import { changeUrlFor } from '../../lib/links'
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 p-2.5 sm:flex-nowrap">
        ...
        {item.pr ? (
          <a
            href={changeUrlFor(repo, item.pr)}
            target="_blank"
            rel="noreferrer"
...
export default function StepItemsTable({
  ...
  stepKey: string
  /** The repository these rows belong to. Required: an item's PR number is
   * per-repository, so a link built without it opens a DIFFERENT, real change
   * under whatever repository the URL happened to name. */
  repo: RepoRef
  items: StepItem[]
