...
class QueueMigrationPending(FoldError):
    """The dispatch queue is still under its pre-sharding name.

    A separate class, not a bare ``FoldError``, because the two callers of
    ``_read_queue`` translate ``FoldError`` into ``bad_step`` and ``bad_item`` --
    both of which blame the request. This condition is not a bad request: it says
    the queue-sharding deploy has not run yet, and the operator is the only one
    who can act on it. Subclassing ``FoldError`` keeps any handler that does not
    know the distinction on the existing visible path rather than a 500.
    """
...
#: carries the repository as a FIELD, while the working set carries it in the
#: PATH. Reading the queue therefore means choosing a file, which is why every
#: reader below takes a repository.
QUEUE_STEM = "gh-autofix-dispatch-queue"

#: Kept byte-identical to the writers' own slug (the scheduled jobs name these
#: files) -- a reader that disagrees opens a file nothing ever wrote. ASCII is
#: spelled out rather than using str.isalnum(), which is Unicode-aware and would
#: diverge from the writers on a non-ASCII repository name.
...
def _repo_slug(repo: str) -> str:
    """One filesystem-safe path segment for ``owner/repo``.

    ``/`` becomes ``__`` FIRST, then anything outside the safe set becomes
    ``-``; the order matters, because ``/`` is itself outside the safe set and
    the catch-all would otherwise turn it into a single ``-``.
    """
    return (
...
def _admits(event_repo: str, wanted: str) -> bool:
    """Does an event belong in the view of ``wanted``?

    It must match exactly, and an event carrying NO repository does not match.

    That last rule is the one worth justifying, because the generous reading is
    tempting and wrong. A pre-stamp event records no repository, and an issue
    number alone does not identify one, so the fold genuinely cannot tell whose it
    was. Admitting it into a scoped view sounds like the humble choice -- count it
    everywhere, since we do not know -- but admission is not where it ends: the
    row is then handed to an enrichment keyed on the issue NUMBER, which reads the
    SELECTED repository's issue cache and the SELECTED repository's queue shard.
    A pre-stamp #42 would therefore appear wearing another repository's title,
    labels, assignees, slot and credit costs, with nothing on screen marking the
    substitution. That is the exact misattribution this scoping exists to remove,
    so admitting the event would reintroduce it through the one door left open.

    Absent beats invented. The ambiguity is DISCLOSED instead of resolved:
    ``unattributedEvents`` is written above this gate and stays whole-trail, so a
    scoped view still reports how many events it cannot attribute -- and therefore
    that its own counters are incomplete -- rather than quietly folding them in as
    if they were this repository's.

    ``wanted`` is REQUIRED: there is no honest default for which repository a reader
    means, so a reader that cannot name one is asking a question the fold cannot
    answer.
    """
    if not event_repo:
        return False
...
def queue_path(*, repo: str, root: Path | None = None) -> Path:
    ...
    surviving positional call would have passed a ``root`` Path where ``repo``
    now sits -- which does not raise where it happens but produces a nonsense
    filename, i.e. an empty queue with no error. Keyword-only turns that into an
    immediate TypeError at every stale call site.

    ``repo`` is REQUIRED, and that is the point. A default would silently hand back
    some other repository's shard to a caller that forgot to say which one it meant --
    plausible-looking rows belonging somewhere else. There is no honest default for
    "which repository's queue": a caller that cannot name one is asking a question
...
def legacy_queue_path(root: Path | None = None) -> Path:
    """The pre-sharding queue name, which carries no repository at all.

    Only ever compared against, never read: this exists so an absent shard can be
    told apart from an absent queue. The pipeline that writes the queue spells
    this same name in its own installer, which renames it to the shard once as a
    deploy step; the two spellings are pinned from both sides because nothing at
    runtime would notice them diverging.
    """
    base = root if root is not None else _workspace()
    return base / f"{QUEUE_STEM}.jsonl"
...
class PipelineFold:
    ...
    #: exists. A SCOPED view therefore excludes them from its step counters, because
    #: admitting them would hand each row to an enrichment keyed on the issue number
    #: and dress it in the selected repository's title, sessions and costs. This
    #: count is always WHOLE-TRAIL, which is what makes that exclusion honest rather
    #: than silent: the view can say how many events it could not place.
    unattributed: int = 0
    ...
    #: the repositories that have events instead of asserting one. Always
    #: WHOLE-TRAIL, even when the fold is scoped: narrowing it would hide the
    #: mixture it exists to disclose.
...
def fold_pipeline(
    *,
    ...
    ``repo`` narrows the STEP COUNTERS to one repository, as ``owner/repo``.

    Two things never narrow, whatever ``repo`` says. ``repos`` and
    ``unattributed`` are a CENSUS: they report which repositories the trail
    contains and how many events predate the jobs stamping one, and a scoped view
    still needs both to disclose what it is not showing. ``total_events`` stays
    whole-trail with them so the three keep describing the same set.

    An event with no repository is EXCLUDED from a scoped view's step counters and
    admitted only by an unscoped fold. It cannot be placed: the record names no
    repository and an issue number alone stops identifying one once a second
    repository exists. Admitting it into a scoped view would not be humility, it
    would be fabrication -- the row is enriched by its issue NUMBER against the
    SELECTED repository's issue cache and queue shard, so it would appear wearing
    another repository's title, sessions and credit costs. The census above keeps
    counting it, so the exclusion is disclosed rather than silent.

    ``repo`` is REQUIRED. Every reader of this fold is looking at one repository, and
    an optional arm would be one a caller reaches for without meaning to: there is no
    honest default for which repository's throughput to report. The whole-trail
    reading is not lost by requiring it -- the census below stays whole-trail on a
    scoped fold, which is where a reader needs it.

    Narrowing is only correct because the dispatch queue is now ONE FILE PER
    REPOSITORY. While it was a single number-keyed file, two repositories sharing
    an issue number collided in the queue before any fold read it, so a filter
    here would have narrowed the step lists while the queue and session joins
    stayed ambiguous -- reading as precision while being wrong. The queue's shard
    is what makes the join as narrow as the filter.
    """
    ...
    for record, ok in _iter_jsonl(text):
        if not ok or record is None:
            ...
            continue
        # Repository attribution serves TWO jobs here, and they are deliberately
        # gated differently.
        #
        # The CENSUS (`repos` / `unattributed`) is disclosure and stays
        # WHOLE-TRAIL even when a repository is selected: its question is "which
        # repositories does this trail contain", and narrowing it would hide the
        # very mixture it exists to report. `total_events` stays whole-trail with
        # it so the two keep describing the same set.
        #
        # The STEP COUNTERS below are the view, and those DO narrow -- a board
        # scoped to one repository must not count another's throughput. Because
        # the census writes sit above the step loop in this single pass, both
        # readings come out of one walk; no second pass is needed.
        ...
        if not _admits(event_repo, repo):
            continue
...
def _read_queue(*, repo: str, root: Path | None = None) -> dict[int, dict[str, Any]]:
    ...
    sharded per repository: issue numbers are per-repository, so a single shared
    file would let one repository's #42 overwrite another's here. The shard is
    what makes this plain-number key safe.
    ...
    An absent shard normally means "no work queued for this repository yet", and
    an empty result is the truth. It is a LIE in one case: the queue-sharding
    deploy has not run, so the entries are sitting under the pre-sharding name.
    Both states look identical from the shard alone, so the legacy name is checked
    before an absent shard is reported as empty, and the difference is raised
    rather than rendered -- an empty board is indistinguishable from a real one
    with nothing in it, which is how stale costs and session history would read as
    simply absent.

    It refuses rather than migrating. Migration is a rename of a settled file,
    performed once by the pipeline's own installer under the lock the dispatcher
    takes; this module cannot see that lock, and a reader that renamed the file
    ...
    text = _read_text_bounded(shard, MAX_JSON_BYTES, "The dispatch queue")
...
def list_step_items(
    ...
    *,
    ...
    ``owner``/``repo`` do two jobs. They locate the local issue cache that
    supplies titles, labels and assignees, and they SCOPE the list: only events
    belonging to ``owner/repo`` are folded, and the queue join reads that
    repository's own shard.

    Both halves narrowing together is the point. While the queue was a single
    file keyed on the issue NUMBER alone, filtering this list would have narrowed
    only the list -- each surviving row then joined against whichever
    repository's entry happened to occupy that number, so a two-repository
    install would read one repository's metadata, sessions and costs under the
    other's name behind a heading asserting it had been scoped. Precise-looking
    and wrong. The queue is now one file per repository, so the join is as narrow
    as the filter and the heading tells the truth.

    An item whose events carry no repository appears on NO scoped list. Those
    events name no repository, and this listing enriches every row by its issue
    number from the selected repository's cache and queue shard -- so including one
    would render it with another repository's title, assignees, slot and costs.
    The count of what was left out stays on the fold's census.
    """
    ...
    # One spelling of the repository for both the event filter and the queue
    ...
    for record, ok in _iter_jsonl(text):
        if not ok or record is None:
            continue
        if not _admits(_event_repo(record), target):
            continue
        name = record.get("event")
        if not isinstance(name, str) or not name:
            continue
...
def list_item_sessions(
    ...
    *,
    ...
    ``repo`` (``owner/repo``) selects which repository's queue shard to read, and
    is what makes the plain-number lookup below correct. Issue numbers are
    per-repository, so before the queue was sharded this join could return
    another repository's slots -- and therefore another repository's sessions and
    credit costs -- for an item that merely shared a number. It is REQUIRED for
    that reason: there is no repository whose shard is the right one to guess.

    The usage shards themselves are NOT per-repository and need no scoping: they
    are keyed by slot, and a slot key is globally unique.
    """
